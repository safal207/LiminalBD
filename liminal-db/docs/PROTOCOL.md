...
## Noetic Layer / Resonant Consensus

Уровень v1.4 добавляет «ноэтическую» репликацию между кластерами. Каждый узел имеет `PeerId` (ed25519) и поддерживает связь через gossip-объявления, сводки состояния и дельты CRDT.

### Реплицируемые структуры

* `HLC` — гибридные логические часы (`phys_ms`, `log`, `node`) с приоритетом по credence при равенстве.
* `LWW<T>` — регистр последнего писателя, использующий HLC.
* `OR-Set<T>` — наблюдаемый набор с семантикой удаления (добавления/удаления помечаются HLC).
* `PN-Counter` — счётчик плюс/минус, агрегируемый по пирами.
* Составное состояние: модели, эпохи, семена, политики, статус семян.

### Пиринговый слой

* `peer.add` — объявление пира `{"cmd":"peer.add","peer":{"url":"ws://...","credence":0.8}}`.
* `peer.list` — запрос текущего списка узлов.
* Gossip-циклы обмениваются `summary` (хэши и счётчики) и `delta` (конкретные изменения).
* Все пакеты подписываются (ed25519), окно анти-replay задаётся в миллисекундах. Ошибки: `UNAUTH`, `FORBIDDEN`, `STALE`, `CONFLICT`.

### Консенсус

* «Мягкий» кворум: сумма credence >= `q` и отсутствуют отрицательные голосования.
* Пакеты:
  * `{"cmd":"noetic.propose","obj":"model|seed|policy","data":{...}}`
  * `{"cmd":"noetic.quorum","q":0.6}` — установка кворума.
* События: `{"ev":"noetic","meta":{"phase":"propose|vote|commit","obj":"model|seed|policy","id":...}}` и `{"ev":"crdt_merge",...}`.
* Конфликты разрешаются CRDT-мерджем; при равных HLC побеждает источник с большей credence, остальные значения остаются в наблюдаемом списке.

### Snapshot/WAL

Снимок включает: список пиров, карту credence, состояние CRDT и незавершённые предложения. В WAL добавляются записи `noetic_propose`, `noetic_vote`, `noetic_commit`, `crdt_merge`.

### Безопасность

Все ноэтические сообщения подписаны. Некорректная подпись → `UNAUTH`, истёкшее окно → `STALE`. Роли управляют правами: `Writer` может предлагать, `Validator` голосовать, `Reader` только наблюдать.
