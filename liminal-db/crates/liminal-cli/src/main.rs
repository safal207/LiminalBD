...
fn format_harmony_event(value: &JsonValue) -> Option<String> {
    ...
    let mirror = match meta.get("mirror") {
        Some(JsonValue::Object(obj)) => {
            ...
            let pattern = obj.get("p").and_then(|v| v.as_str()).unwrap_or("-");
            format!("{}:{:.3}", pattern, strength)
        }
        _ => "-".into(),
    };
    let load = (strength - delta_strength).clamp(0.0, 1.0);
    Some(format!(
        "HARMONY state={} load={:.3} strength={:.3} latency={:.1}ms entropy={:.3} d_str={:.3} d_lat={:.1} pattern={} mirror={}",
        status, load, strength, latency, entropy, delta_strength, delta_latency, pattern, mirror
    ))
}
...
fn render_harmony_snapshot_line(snapshot: &HarmonySnapshot) -> String {
    let state = snapshot.status.as_str().to_uppercase();
    let load = (snapshot.metrics.avg_strength - snapshot.delta_strength).clamp(0.0, 1.0);
    let strength = snapshot.metrics.avg_strength;
    let latency = snapshot.metrics.avg_latency;
    let entropy = snapshot.entropy_ratio;
    ...
    let mirror = snapshot
        .mirror
        .as_ref()
        .map(|m| format!("{}:{:.3}", m.pattern, m.strength))
        .unwrap_or_else(|| "-".into());
    format!(
        "HARMONY state={} load={:.3} strength={:.3} latency={:.1}ms entropy={:.3} d_str={:.3} d_lat={:.1} pattern={} mirror={}",
        state,
...
mod tests {
    ...
    fn harmony_event_formatter_reports_status() {
        ...
        assert!(formatted.contains("state=DRIFT"));
        assert!(formatted.contains("load="));
        assert!(formatted.contains("mirror=cpu/load:-0.300"));
    }
    ...
    fn render_harmony_snapshot_outputs_state() {
        ...
        assert!(line.contains("state=DRIFT"));
        assert!(line.contains("load="));
        assert!(line.contains("pattern=cpu/load"));
        assert!(line.contains("mirror=cpu/load:-0.220"));
    }
}
