...
function typeForProperty(property: any): string {
  if (property.const !== undefined) {
    return JSON.stringify(property.const);
  }
  if (property.enum) {
    return property.enum.map((v: string) => JSON.stringify(v)).join(' | ');
  }
...
function renderAnonymousType(schema: JsonSchema): string {
...
function renderUnion(name: string, members: string[]): string {
  return `export type ${name} = ${members.join(' | ')};`;
}

async function loadSchema(name: string): Promise<SchemaBundle> {
...
async function generateTypes() {
  ...
  const lines: string[] = [
    '// This file is auto-generated by protocol/codegen.ts. Do not edit manually.',
    '',
  ];
